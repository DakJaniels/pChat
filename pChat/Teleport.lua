...
EVENT_MANAGER:RegisterForEvent("pChat_GuildCache", EVENT_GUILD_MEMBER_ADDED, OnGuildMemberAdded)
EVENT_MANAGER:RegisterForEvent("pChat_GuildCache", EVENT_GUILD_MEMBER_REMOVED, OnGuildMemberRemoved)
EVENT_MANAGER:RegisterForEvent("pChat_GuildCache", EVENT_PLAYER_ACTIVATED, OnPlayerActivated)
...
local function isPlayerInAnyOfYourGuilds(displayName, possibleDisplayNameNormal, possibleDisplayName, p_guildIndex, p_guildIndexIteratorStart)
    ...
    if not next(guildMemberCache) then
        refreshAllGuildCaches()
    end
